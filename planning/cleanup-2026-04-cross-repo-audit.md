...
| ID | Repo | Question | Status |
|---|---|---|---|
| **E** | apcore-rust | Should Rust SDK support YAML annotation overlays at all? | **DECLINED 2026-04-09 — feature, not cleanup.** Spec §4.13 only mandates field-level merge **if implemented**; does not require YAML overlay support. Adding it requires a new file scanner, schema loader, and changes to the `Discoverer` trait (currently accepts pre-built `DiscoveredModule`). Needs spec RFC + design doc deciding whether YAML overlays are SHOULD or MUST across all SDKs. Out of scope for cleanup |
| **F** | apcore-rust | Drop `Context::serialize()` method in favor of serde trait only? | **DECLINED 2026-04-09 — public API.** Investigation found 16 call sites in `tests/conformance_test.rs` and `tests/context_serialization_test.rs` plus documentation in `CHANGELOG.md:141` listing it as a v0.17.x feature. The "mild redundancy" with the serde trait is intentional: `.serialize()` produces the spec-defined `_context_version: 1` envelope, while raw serde traits expose internal struct shape. They serve different purposes |
| **G** | apcore-typescript | Is `config.ts` worth migrating to a typed Config like Rust? | **DEFERRED — already documented as defer-until-pain-point in this same file (P2 row above).** TS already has a single 924-line `Config` class supporting both modes (better than Python's two-class state). Marginal value vs Rust-style typed struct migration. Re-open when a concrete pain point appears |
| **J** | apcore-typescript | How should `module()` factory generate auto-IDs to comply with spec §5.11.6 `{module_path}.{name}`? | **DECLINED 2026-04-09 — documented design + test asserts current behavior.** Verification 2026-04-09: `apcore-typescript/tests/test-decorator.test.ts:116-123` explicitly asserts `fm.moduleId === 'anonymous'` for the missing-`id` path, and `apcore-typescript/planning/decorator-bindings/plan.md:15-17` documents the `makeAutoId('anonymous')` default as the deliberate design (rationale: JS lacks runtime `__module__` reflection, so spec §5.11.6's `{module_path}.{name}` rule cannot be implemented identically to Python). Changing this is a maintainer-level design call, not a cleanup. If the maintainer chooses to fix this, the recommended approach is **throw on missing `id`** with an error pointing at spec §5.11.6. The alternative `options.execute.name` fallback only works for non-arrow named functions and would create silent partial-fix behavior. Needs explicit maintainer decision before any code change |

### P3 — Multi-repo design rounds (do not start without spec/design doc)

| ID | Scope | Status |
|---|---|---|
| **H** | python + typescript errors collapse | **DEFERRED — needs design doc.** Both languages have 42-class hierarchies; Rust's single `ModuleError` + `ErrorCode` enum is the target. Cannot be done in a single session because: (a) `FeatureNotImplementedError`/`DependencyNotFoundError` are locked into `apcore-python/tests/test_public_api.py:425-426` as published API surface, (b) both repos document the 42-class set in CHANGELOG.md as v0.17.0 features, (c) downstream code likely uses `isinstance` / `instanceof` checks that would break, (d) needs a deprecation cycle with warnings, not a clean delete. Belongs to a future round with a written design doc and three-repo lockstep PR |
| **I** | apcore-python config typed migration | **DEFERRED — explicit defer rationale documented in `cleanup-2026-04.md:52`.** Reference: `apcore-rust/src/config.rs:152-200`. Estimated 500-800 LOC, breaks ~10 test files, requires Pydantic v2 model design. Owner: a future round with a written design doc first. Python team explicitly chose NOT to attempt this under "solve everything" pressure because the risk-to-value ratio was wrong |
