...
- **`TaskStoreError(code=TASK_STORE_UNAVAILABLE)` exists (spec v1.50.0, async-tasks.md "`TaskStoreError` must exist before it can be raised", D-92).** The page declares it on **eight** surfaces — every `TaskStore` method and every `AsyncTaskManager` method that touches the store — and no SDK defined it: neither the class nor the code appeared in apcore-python, apcore-typescript or apcore-rust. A declared error type no implementation can raise is one no caller can catch, which is the "declared surface reaches no mechanism" shape §9.1.3 forbids for configuration keys, applied to an error contract. `TaskStoreError` is now a `ModuleError` subclass in `apcore.errors`, `TASK_STORE_UNAVAILABLE` is in `ErrorCodes` (and therefore framework-reserved against A17 module registration), and both are exported from the package root. The bundled `InMemoryTaskStore` cannot fail and never raises it — which is exactly why the type must be **exported** rather than merely raised internally: the hosts who need it are the ones writing the network-backed stores the contract was written for.

- **`ACL.check_access` / `async_check_access` / `check` / `async_check` accept a `projection` keyword.** See the governance-projection entry under Fixed.

### Changed

- **`AsyncTaskManager.start_reaper` raises `ModuleError(code=REAPER_ALREADY_RUNNING)` instead of a bare `RuntimeError` on a double start (async-tasks.md "Contract: AsyncTaskManager.start_reaper", A-C-004).** apcore-rust already raised a typed `ModuleError` with `ErrorCode::ReaperAlreadyRunning` for this case; apcore-python and apcore-typescript raised an untyped exception, so a caller writing `except ModuleError` caught Rust and silently missed the other two. The spec's Errors section previously read "None" for `start_reaper`, which all three SDKs already contradicted; it has been corrected to document the double-start guard. **Caller-visible type change:** code catching `RuntimeError` around `start_reaper` must catch `ModuleError` (or the new `REAPER_ALREADY_RUNNING` code) instead.

- **The `config_bus.get.input.key.empty` contract test asserted a rule D-74 deleted (spec, D-74).** It was `@pytest.mark.skip`ped with "contract says `Config.get('')` is rejected ... but this SDK returns the default", a reason written before D-74 removed that row and recorded that it "described behaviour no SDK has ever had". The implementation was already correct; the test was asserting the retracted rule and the skip kept it green. It now asserts the D-74 behaviour — an empty key resolves no value and returns the default, exactly like any other absent key.

- **The deprecation-warning dedupe key carries the notice, and survives `unregister` (spec v1.59.0, D-89).** `unregister` used to drop the marker (`_forget_deprecation_warnings`) so a re-registration carrying a new `x-deprecation` block still warned. But `watch()` re-runs discovery as an unregister + re-register, so that also **re-warned for every deprecated module on every hot reload** — the traffic-proportional spam D-89 exists to prevent, arriving through the door its wording did not close. The key is now `(module_id, version, x-deprecation block)` and is never cleared: a re-registration carrying the **same** notice is silent, one carrying a **changed or newly added** notice warns. Blocks compare by canonical value (sorted keys, no incidental whitespace), so two structurally equal notices dedupe regardless of how they were built. This SDK already emitted from `get_definition`, which v1.59.0 confirms as the emission point; that is unchanged and now asserted.

- **An unknown extension point is an error, and it now carries a code (spec v1.51.0, extension-system.md, D-108).** `ExtensionManager.register` / `get` / `get_all` / `unregister` raised a builtin `KeyError` for a point name that is not registered. `KeyError` is what a plain dict miss raises: it carries no apcore error code, so a caller could not branch on it, it escapes every `except ModuleError` handler, and the cross-language assertion for it could do no better than match on message text. All four doors now raise `InvalidInputError(code=GENERAL_INVALID_INPUT)` through one `_require_point` path, which is the code apcore-rust has always used for this and the one apcore-typescript changes to. The other half of D-108 is unchanged and now asserted: a point that IS registered and currently holds nothing is **not** an error — `get` returns `None`, `get_all` returns `[]`, `unregister` returns `False`. **Caller-visible type change:** `ModuleError` is not a `KeyError` subclass, so code catching `KeyError` around these calls must catch `InvalidInputError` (or `ModuleError`) instead.

- **`APCore` adopts a supplied Executor's Registry instead of building a second one (apcore-client.md "Contract: APCore.with_options", CLI-1).** `__init__` built `registry or Registry()` and then, when the caller supplied an `executor`, ignored the executor's own registry entirely — so the client's write path (`register`, `@client.module`, `discover`) and the executor's read path addressed two different objects. Measured before the fix: `APCore(executor=Executor(registry=my_reg)); client.register("a.b", mod); client.call("a.b")` raised `MODULE_NOT_FOUND`, `list_modules()` advertised a module that could not be called, and `client.disable("x")` passed the `_require_sys_modules` guard and then failed on `system.control.toggle_feature`, because system modules were registered into the client's copy while the executor looked in its own. The contract already said this — `registry` is "Ignored when executor is also provided (the executor's own registry is used instead)" — and apcore-rust already did it. A `registry` argument passed alongside an `executor` is now ignored, as documented.

- **The client's `Config` reaches the Registry it creates (apcore-client.md "Contract: APCore.discover", CLI-3).** The auto-created `Registry()` was built with no config although `Registry.__init__` accepts one, so `extensions.root`, `extensions.roots`, `extensions.max_depth`, `extensions.follow_symlinks`, `extensions.ignore_patterns` and `id_map.overrides` were **all inert through the client door**: `_scan_params` returned hardcoded `(8, False, [])` and the roots fell back to the built-in `./extensions`, so `client.discover()` scanned a directory the operator never configured and raised `CONFIG_NOT_FOUND` for it. This is the §9.1.3 shape again — the mechanism existed in `Registry` and no `Config` reached it from this door.

- **A per-step `timeout_ms` takes the ordinary step-error path (core-executor.md "Step Metadata", EXE-006).** `PipelineEngine.run` had a dedicated `except asyncio.TimeoutError` clause **ahead of** the general handler that raised `PipelineAbortError(MODULE_TIMEOUT)` without calling `_invoke_on_step_error`; the Executor's abort branch then re-raised without `_recover_from_call_error`. So a step timeout skipped `on_step_error`, could not be resumed by a recovery value, and never reached module-level middleware `on_error` — only `ignore_errors` worked, via that clause's own copy of the logic. `timeout_ms` is documented as a plain per-step timeout ("0 = no limit") with no exemption, and apcore-typescript and apcore-rust both route it through the normal path. **The caller-visible error is unchanged:** the timeout now travels as a `PipelineStepError` carrying a typed `ModuleTimeoutError(code=MODULE_TIMEOUT)` cause, which the Executor unwraps to the same `ModuleTimeoutError` as before — and which is what makes the recovery chain run.

- **`ExecutionStrategy.replace` and `configure_step` reject a duplicate step name (design-execution-pipeline.md, EXE-007).** "Step names MUST be unique within a strategy" is checked by the constructor, which never runs again, and by `insert_after` / `insert_before` on their own doors; `replace` and `configure_step` assigned straight into `self.steps` and rebuilt the index. Two identically-named steps could therefore coexist, and `_rebuild_index` maps the name to the **later** one — silently, so a subsequent `insert_after("module_lookup", ...)` anchored on the impostor, `remove()` targeted the wrong entry, and a `skip_to` resolved past the real step. Both doors now raise `StepNameDuplicateError(code=STEP_NAME_DUPLICATE)`, matching apcore-rust's `validate_replacement_name`. Replacing a step with one of the same name is still allowed — that is what `configure_step`'s idempotence means — and so is renaming to a free name.

- **The three `system.control.*` modules declare `idempotent` explicitly (system-modules.md, SYS-19).** All three took the `ModuleAnnotations` default of `False`. system-modules.md states the value per module: `update_config` false, `reload_module` false, `toggle_feature` **true** ("toggling to the current state produces the same outcome"). Two matched by accident; `toggle_feature` did not, so a caller reading its annotations was told a no-op re-toggle was not safe to repeat.

- **A bulk `system.control.reload_module` failure is fatal (SYS-17).** `_execute_bulk`'s per-module `except Exception` logged and continued, and `_reload_one` unregisters a module **before** re-discovering it — so a re-discovery failure returned `{"success": true, "reloaded_modules": []}` for a reload that unregistered every matched module and restored none. That is the worst possible pairing of an outcome with a report: the caller has no reason to look further. It now raises `ReloadFailedError` naming the module that stopped the run, as apcore-typescript and apcore-rust both do. Modules already reloaded in the same pass stay reloaded; the partial list is simply not reported as a success.

- **`apcore.registry.callback_errors` renamed to `apcore_registry_callback_errors_total` (OBS-007).** The Prometheus exposition format admits `[a-zA-Z_:][a-zA-Z0-9_:]*` only, and `MetricsCollector.export_prometheus` writes the counter name verbatim — so one dotted sample made the **whole scrape** unparseable and every other apcore metric was lost with it, not just this one. No other SDK carries this metric, so nothing depended on the old spelling. The labels (`event`, `module_id`, `error_type`) are unchanged.

- **`guard_call_chain` rejects a non-positive limit with a typed apcore error (spec v1.49.0 §"Contract: guard_call_chain" `### Errors`, D-84).** It raised a builtin `ValueError`, which escapes every `except ModuleError` handler and carries no code from the registry. The path that made it visible: `Executor.validate()`'s error mapper turns an untyped exception into `error.code == type(exc).__name__`, so a misconfigured `executor.max_call_depth: 0` surfaced through preflight as `error.code == "ValueError"` — a string that is not an apcore error code at all, and one no cross-language caller can branch on. Both floors now raise `InvalidInputError(code=GENERAL_INVALID_INPUT)`, which is what apcore-rust already did and what apcore-typescript changes to. **Caller-visible type change:** `ModuleError` is not a `ValueError` subclass, so code catching `ValueError` around `guard_call_chain` must catch `InvalidInputError` (or `ModuleError`) instead.

- **A deprecation warning fires at most once per `(module_id, version)` per registry instance (D-89).** `_log_deprecation_warning` fired on every `get_definition`, and `get_definition` is a **read** that hosts call in loops — so the volume of an advisory was proportional to traffic, which is how operators learn to filter it out and then miss the next one. The dedupe set mirrors apcore-typescript's `_deprecationWarned`. It is cleared for a module when the module leaves the registry (`unregister`, and the hot-reload path's `_inline_unregister`), because the replacement may carry a different or newly added `x-deprecation` block and a stale entry would silence it.

- **`Registry.register` reports validation failures in the spec's order (registry-system.md "Side Effects (ordered)" steps 2-5, D-86).** The list previously named neither the structure check nor the custom validator, and the three SDKs drifted into three orders: Python ran validator → streaming, apcore-typescript duplicate → validator, apcore-rust streaming → validator → duplicate. The same call — a module that is simultaneously streaming-malformed, validator-rejected and a duplicate — therefore reported a different error in each. The order is now `module_id` → structure/streaming → custom validator → duplicate, matching apcore-rust: intrinsic-then-extrinsic, because what is wrong with the module must be fixed either way whereas a duplicate ID may just mean the author picked the wrong name. One side effect worth stating, unchanged by the reorder: a stateful custom validator **is** invoked for a registration that later fails the duplicate check.

- **`AsyncTaskManager.shutdown()` attempts every cancellation before it reports (spec v1.52.0, async-tasks.md, D-122).** D-81 made `TaskStoreError` reach the caller and said nothing about the tasks not yet reached, so the three SDKs split while implementing it on the same day: this one and apcore-rust stopped at the first failing `cancel()`, apcore-typescript attempted all. Stopping early is the worse half of an asymmetry — when the *store* is unreachable both strategies cancel nothing and fail-fast merely arrives there sooner, but when *one task* fails it leaves every remaining active task uncancelled, and an uncancelled task in a shared store holds a `max_tasks` slot for every manager pointed at that store and outlives the process that could have cancelled it. `shutdown()` now cancels every active record and raises the **first** error afterwards, so the caller still learns the shutdown was not clean. The reaper is stopped before the loop, as before. This does not make `shutdown()` slower to hang: it is **already an unbounded wait by contract** — it waits for in-flight tasks and takes no timeout parameter — so a caller needing a bound must already impose one from outside, and that bound covers the cancellation loop too.
...
- **SECURITY: a governance requirement declared in a metadata document gated nothing (spec v1.54.0,
  ...
  requirement declared there reached `get_definition()` and the `system.manifest.*` entry — both reporting
  ...
  `apcore.schema.governance_union`. **Two behaviour changes worth stating:** a metadata `requires_approval: false` no longer
...
- **SECURITY: a `$ref` into an external schema could bind to the caller's definitions, including past
  ...
  second base — the schema node, after the file root — and did not say how far it travels. `_fallback_document` returned the cached inline node for any local reference whose
  first lookup went to a real file, without asking WHICH file.
...
- **`global_deadline` is epoch seconds, belongs to the call tree, and is recomputed unconditionally (spec v1.50.0 core-executor.md "`global_deadline` Representation and Lifetime", D-99 / D-101 / D-102).** Three defects in one four-line block.
  **D-99 — the clock.** `BuiltinContextCreation` wrote `time.monotonic() + timeout/1000` and the execute step compared against `time.monotonic()`; the field is documented as **epoch seconds** and is a public `Context.create` parameter, so a caller following the spec writes `time.time() + budget`. Against a monotonic basis that is roughly `1.8e9` compared against roughly `1e5`: the deadline never fires and the call runs with **no budget at all** — silent, and fail-open. Both comparison sites and the streaming deadline check now read `time.time()`.
  **D-101 — the lifetime.** The deadline was written onto `base_ctx`, which **is** the caller's `Context` (`bind_executor` mutates it in place). Reusing one Context across successive top-level `Executor.call()` invocations is explicitly blessed, and a budget pinned to that object made the second call inherit the first call's remaining time — or fail immediately, having already expired. It is now computed onto the context the pipeline derives for this call; `Context.child()` already carries it down the tree, so nested calls are unaffected.
  **D-102 — the recomputation.** The guard was `not base_ctx.call_chain and global_deadline is None`. A Context arriving from another process carries a non-empty `call_chain` **by definition**, so the `is_root_call` conjunct inverted "the receiving Executor MUST recompute" exactly where it applies and every cross-process sub-tree ran with no global budget. The conjunct is gone; `global_deadline is None` alone already preserves an in-process caller's explicit value.

- **`_approval_token` no longer reaches policy resolution from `Executor.validate()` (§7.9.6 rule 5, APR-2).** Rule 5 is a MUST and states why §7.4's "before passing to subsequent steps" does not cover it: policy resolution happens *inside* Step 5, ahead of any subsequent step, so an implementation can satisfy §7.4 literally and still hand the framework token to the policy — into the audit trail and the `apcore.policy.override` payload. The approval gate stripped at its call site; `validate()` passed `pipe_ctx.inputs` intact and nothing downstream stripped either. The guard now sits in **two** places on purpose: `ExecutionPolicy.resolve` strips (covering every door into the built-in rules, and exported as `strip_approval_token`), and `validate()` strips at its call site because the surface rule 5 protects is a host policy that **overrides** `resolve` — which `resolve`'s own docstring names as how a host consults the call site, and which is entered before any strip inside the base implementation runs. Neither strip mutates the caller's mapping.

- **A local `#/…` `$ref` resolves against the schema FILE root, then the schema node (PROTOCOL_SPEC Algorithm A05 step 4a, D-104).** `SchemaLoader.resolve` passed `current_file=None`, so local pointers resolved **only** within the `input_schema` / `output_schema` dict. The consequence: the spec's own §4.11 example — `definitions:` as a top-level sibling of `input_schema` in the schema FILE — raised `SCHEMA_NOT_FOUND` here and loaded on apcore-rust alone, while `#/$defs/User` nested inside `input_schema` loaded here and not there. Each SDK rejected what the other accepted, so **no schema file containing a local `$ref` loaded in all three**. Both layouts are now normative and both load: the file root is tried first and the schema node is the fallback, which cannot collide because a pointer either resolves at the file root or it does not. `SchemaDefinition` gained a `source_path` field to carry the file to the resolver; a definition built in memory has `None` there and behaves exactly as before. A bare `#` / `#/` is untouched — it still denotes the schema being resolved and stays a lazy self-reference (§4.15.2), because a file root is a schema *file*, not a schema. **Side effect:** `SchemaDefinition.definitions` becomes live. It was collected from the file's top level and read by nothing, precisely because the references that would have used it could not resolve.

- **`Identity.roles` is copied and frozen at construction (identity-system.md, IDN-2).** `__post_init__` copied `attrs` and left `roles` as whatever it was handed, although the annotation says `tuple[str, ...]` and the page requires an immutable sequence. Measured before the fix: `roles = ["admin"]; ident = Identity(id="u", roles=roles); roles.append("root")` left the identity holding `['admin', 'root']` — a list — so a later ACL `roles` condition granted a role nobody assigned. `None` now becomes `()`, and any other sequence becomes a `tuple`.

- **`Context.deserialize` raises a typed error on a malformed wire identity (CTX-1).** It indexed `id_data["id"]` unguarded, so `{"identity": {"type": "user"}}` raised a bare `KeyError: 'id'` and `{"identity": "root"}` raised `TypeError: string indices must be integers` — out of a method documented to best-effort proceed, and neither catchable as a `ModuleError` nor carrying a registry code. `identity`, `identity.id`, `identity.roles` and `identity.attrs` are now shape-checked and a violation raises `InvalidInputError(code=GENERAL_INVALID_INPUT)`. apcore-typescript coerces every field and apcore-rust drops the identity; both report rather than crash on a builtin.

- **The governance projection is scoped to one ACL evaluation (§6.1.8, CTX-2).** Step 3 wrote `GovernanceProjection.of(inputs)` onto `ctx.context` — the execution `Context`, which is handed to module code and which a caller may explicitly reuse across successive top-level calls — and left it there. A module or middleware holding that Context and calling `acl.check_access(..., ctx)` afterwards therefore evaluated an `arguments` condition against the **previous** call's argument key set. §6.1.8 rule 1 makes the projection a property of the call being checked, and rule 4 blesses passing it as an argument, which is what apcore-typescript and apcore-rust do. Step 3 now writes it onto the per-call `PipelineContext`; Step 4 passes it to `check_access` / `async_check_access` through a new keyword-only `projection` parameter, scoped for the duration of that one evaluation. An ACL object predating the keyword still works — the ACL step falls back to the positional call — and `Context.governance_projection` remains readable for a host that carries it there itself, the other shape rule 4 allows. Not supplying a projection leaves an `arguments` condition UNEVALUABLE, never vacuously satisfied.

- **Wrapping a filtered subscriber in a circuit breaker widened it to catch-all (EVT-001).** `CircuitBreakerWrapper` declared no `event_pattern`, so `EventEmitter._get_event_pattern` fell back to `"*"` — and an event type the operator had deliberately excluded was POSTed to the webhook. `subscriber_id` and `subscriber_type` had the same shape: the DLQ payload and every emitter log line named the wrapper's `repr()` (a heap address) instead of the subscriber the operator configured. All three are now forwarded from the wrapped subscriber, as apcore-rust does.

- **A subscriber with no declared id got `repr(subscriber)` in the DLQ payload (event-system.md, EVT-004).** The page requires the emitter to "MUST generate a stable identifier"; the fallback embedded a heap address, which is neither stable across runs nor matchable to anything an operator can see. `_next_subscriber_id` already existed for exactly this and was called only by the built-in subscriber classes. `subscribe()` now assigns a generated `{type}-{N}` id when the subscriber declares none, once, and never overwrites a declared one.

- **Hot reload dropped `_module_meta`, so every reloaded module came back with an empty descriptor.** `Registry._handle_file_change` unregisters the old instance from every internal store and re-inserts the new one; the re-insert wrote `_modules`, `_lowercase_map`, `_versioned_modules` and `_versioned_meta` but not `_module_meta`, which the unregister had popped. Measured before the fix: touch a watched module file and `get_definition(id)` answers `description=""`, `version="1.0.0"`, `tags=[]`, `annotations=None`, `dependencies=[]` for a module that reported all of them correctly a moment earlier, while `get_module_metadata(id)` answers `{}`. The dependency loss is the one with teeth — `ReloadModule._topo_sort_modules` reads `get_module_metadata()["dependencies"]` to order a `path_filter` reload, so a topological reload order silently degraded to an alphabetical one. Phase 4 now rebuilds the entry with `merge_module_metadata(instance, {})` inside the same lock as the other four map writes. **Known gap, unchanged by this fix:** the hot-reload path still does not call `on_load()` on the new instance, where apcore-rust's re-discovery does; routing Phase 4 through the deferred-publish helper `_register_in_order` uses would also have to answer what happens when `on_load()` fails *after* the old instance is already gone, so it is left for a separate change.

- **A scalar `dependencies:` aborted the entire `discover()` pass.** `parse_dependencies('foo')` raised `AttributeError: 'str' object has no attribute 'get'` — a bare non-apcore exception, so one hand-written `_meta.yaml` took down discovery for every module beside it rather than being skipped. A truthy scalar passed the falsiness guard and the `for dep in deps_raw` loop then iterated *characters*. Nothing validates this key and no schema describes it, and the value comes off the filesystem. It now degrades to `[]` with a warning, and a list entry that is not a mapping is skipped with a warning rather than raising, matching apcore-typescript (`Array.isArray`) and apcore-rust (`.as_array()`), which both carry comments saying a scalar can arrive here and must never crash a reload. `merge_module_metadata` narrows its `dependencies` slot to lists the way it already narrows `tags`, `examples` and `metadata`. Both reachable call sites are covered: `_resolve_load_order` (discovery) and `get_definition`.

- **`cancel()` returned `False` for an active task with no in-process handle.** The spec's cancel Contract defines exactly two `False` cases — the task did not exist, or it had already reached a terminal state. A missing local `asyncio.Task` handle was a third one, and it is the case the persistent `TaskStore` feature exists for: every PENDING/RUNNING record left behind by a previous process was uncancellable for good, and each one kept consuming the `max_tasks` active budget. When no local handle exists the interrupt step is now skipped and the active -> CANCELLED store write still happens, as in apcore-rust where a missing `JoinHandle` is not an early return. `shutdown()`'s compensating branch for exactly this case is now redundant and was removed — it routes every active record through `cancel()`.

- **The async-task runner clobbered an out-of-band CANCELLED with FAILED, and resurrected terminal records as PENDING.** The runner fetched its `TaskInfo` once and reused the object for every subsequent write; only the COMPLETED write re-checked the store first. With a shared or persistent `TaskStore`, process B cancelling task T while process A's attempt failed yielded FAILED in Python where apcore-rust yields CANCELLED, and the retry path wrote `status = PENDING` straight over a terminal record — a `completed -> *` transition, which protocol-spec.md §5.8 forbids. Every status write (RUNNING, PENDING-retry, COMPLETED, FAILED, CANCELLED) now consults the stored record first and abandons the write when it is already terminal, and `info` is re-fetched at the top of each retry-loop iteration instead of being carried across the backoff sleep. Ported from apcore-rust's `save_terminal_if_not_cancelled`, with one difference the host language forces: the check runs *before* the in-memory mutation, not just before `save`, because `InMemoryTaskStore` hands out the live `TaskInfo` and mutating first would corrupt the stored record even when the write is abandoned.

- **The reaper never called `TaskStore.list_expired`.** It swept through `cleanup()`, which lists the whole task set, so the `list_expired` method declared in the `TaskStore` Protocol and implemented in `InMemoryTaskStore` was reached by nothing — the "declared surface reaches no mechanism" shape §9.1.3 closes, and against async-tasks.md §1.3, which says the reaper SHOULD call `store.list_expired(before=now - ttl_seconds)`. Two consequences: a custom store whose `list_expired` is the only efficient expiry query (Redis `ZRANGEBYSCORE`, SQL `WHERE`) was bypassed and its entire task set pulled into the process on every sweep; and the predicates differ, because `cleanup()` falls back to `submitted_at` when `completed_at` is `None`, so a terminal record without a `completed_at` was reaped here and leaks in TypeScript and Rust. The sweep now runs `list_expired(time.time() - ttl_seconds)` and deletes what comes back, matching apcore-typescript and apcore-rust; a raising store warns and the next tick retries rather than killing the loop. `cleanup()` is unchanged and stays the public full-scan surface with its documented `submitted_at` fallback.

- **The §10.6.1 sensitive-key rule was implemented twice.** `apcore/observability/context_logger.py` carried its own `_normalize_key_for_match` / `_compact_key_for_match` / `_key_matches_sensitive` beside the canonical `_normalize_for_match` / `_compact_for_match` / `_key_matches` in `apcore/utils/redaction.py` — `_compact_key_for_match` byte-identical including its docstring, `_key_matches_sensitive` differing only by an absent empty-list early return that the loop makes unreachable anyway. §10.6 requires the rule to hold on **both** the executor capture-point surface and the log-emission surface, which is what makes a second copy a second rule; the sibling value matcher in the same file had already been collapsed for exactly that reason after the two copies drifted into redacting `amount: 42` on one surface and not the other. The log-emission surface now imports the one matcher, as apcore-typescript and apcore-rust each do.

- **Module-level metrics extraction was implemented twice.** `sys_modules/health.py` read the call counters by exact keyed-tuple lookup while `middleware/platform_notify.py` scanned the labels, and the "find `labels_key` + `total_count` for this module" preamble ahead of `estimate_p99_latency_ms` was duplicated in both. Both were correct, and the same split in apcore-rust is where the second copy read the wrong label key and pinned the error rate at 0 permanently. `module_call_counts`, `module_error_rate` and `module_latency_ms` now live beside `estimate_p99_latency_ms` in `apcore/observability/metrics.py` and both call sites import them, mirroring apcore-typescript's `metrics-utils.ts`. The label-scan form is the one kept, so a call recorded with a `status` other than `success`/`error` now counts toward the health module's total instead of being dropped.

- **Two dead private constants removed.** `apcore/utils/normalize.py`'s `_CASE_BOUNDARY`, a compiled PascalCase/camelCase splitting regex, had exactly one occurrence ecosystem-wide — its own definition — beside the hand-rolled `_to_snake_case` loop that actually does the job and is covered by the A02 conformance fixture. `apcore/observability/context_logger.py`'s `_REDACTED` was defined and never read while the module hard-coded the same literal twice below it; those two sites now use the canonical, publicly exported `REDACTED_VALUE` from `apcore/utils/redaction.py`.

- **Three docstrings asserted peer APIs that do not exist.** `apcore/events/__init__.py` and two places in `apcore/sys_modules/registration.py` claimed parity with TypeScript `registerSubscriberFactory` and Rust `register_factory`; neither name appears anywhere in either SDK. The real exports are `registerSubscriberType` and `register_subscriber_type` — the normative name in event-system.md, which all three SDKs satisfy. Only the claims were wrong, so only the claims changed; no function was renamed.

- **`ACL.add_rule` did not clear the §6.5 warning-dedupe set.** The "conditional rule with no context" warning is deduped per rule **index**, and `add_rule` inserts at index 0, shifting every existing rule down by one — so a stale marker suppressed the warning for a different rule than the one it was recorded for. `reload()` already cleared the set; `add_rule` now does too, as apcore-typescript does in both places.

- **`ACL.remove_rule` did not clear the §6.5 warning-dedupe set either (D-88).** The same index-keyed set, the same failure in the other direction: `remove_rule` pops at index `i` and shifts every rule after it *up* by one, so the marker left by the removed rule silenced the warning for whichever rule inherited its index. D-88 states the rule the `add_rule` fix only satisfied by accident — **any** operation that inserts, removes or reorders rules must clear the set; deduping by index without clearing does not conform, while not deduping at all (as apcore-rust does) still does. A `remove_rule` that matches nothing shifts no index and leaves the set alone.

- **An over-length module ID reached the pipeline and came back as `MODULE_NOT_FOUND` (core-executor.md "Contract: Executor.call", D-75).** The Contract has always said "Empty / over-length / malformed IDs MUST be rejected before the pipeline context is constructed", and all three SDKs checked only the empty-and-pattern half; `MAX_MODULE_ID_LENGTH = 192` existed in all three but was enforced one step later, in the registry. A 300-character well-formed ID therefore built a `PipelineContext`, ran the lookup, and reported that no such module was registered — true, but not the reason, and not the error the entry guard declares. `Executor._validate_module_id` now carries the bound and raises `InvalidInputError(code=INVALID_MODULE_ID)`; the bound is inclusive, so exactly 192 characters still reaches the registry.

- **`Registry.describe` returned a Python repr for a module that supplies its own `describe()` (PROTOCOL_SPEC §12.2, registry-system.md "Contract: Registry.describe", D-77).** The optional `Module.describe()` is declared to return an introspection **mapping** (`{description, input_schema, output_schema, annotations}`), and the registry passed it through `str()` — so a module that implemented the documented method got `"{'description': '...', 'input_schema': {...}}"` as its human-readable description, which is neither a description nor portable across the three SDKs. §12.2 is corrected to declare `describe → String` (it declared `ModuleDescription`, which matched none of the three implementations), and the override is now returned only when it **is** a string, which is the author's deliberate text; a mapping, a `None`, or anything else falls through to the generated envelope. Matches apcore-rust, which was the only SDK doing the shape check. `get_definition` remains the structured accessor.

- **A hot-reloaded module became visible before its `on_load()` had run — in fact without it running at all (spec v1.52.0, registry-system.md "Hot Reload", D-123).** `Registry._handle_file_change` wrote the re-imported instance straight into `_modules`, `_versioned_modules`, `_lowercase_map` and `_module_meta`. Every other registration door in this SDK runs the hook first and publishes only on success — `discover()` in its Phase 2/3 split, `register()` under `Contract: Registry.register` Side Effects step 8 — and that contract is scoped to `register`, so the watch path broke no stated rule while publishing a module that was visible, callable and never initialised. That is strictly worse than an absent module: it fails at call time, inside whatever the hook was supposed to have set up, with a stack trace pointing nowhere near the reload that caused it. The publish is now deferred behind `_invoke_on_load` (which also emits the canonical `apcore.registry.module_load_failed`, as every other door does), and recovery follows **D-112 rules 2-4**: the previous instance is restored and *its* `on_load` re-run — Phase 3 already ran its `on_unload`, so republishing it without a fresh load would leave a visible-but-torn-down module — and if that restoring load fails too the module stays unavailable. Neither peer SDK was affected: apcore-rust's `watch_loop` re-runs `discover_internal()`, which invokes `on_load` before registration, and apcore-typescript's `_handleFileChange` unregisters and emits `file_changed` without re-registering at all.
...
- **The executor's ACL step is pinned to the ASYNC path (spec v1.50.0, D-105).** No behaviour change
  ...
  the door, and a registry that accepts a handler is not evidence anything calls it. `tests/test_executor_async_acl_path.py`
...
- D-122 and D-123 are pinned red-first. `tests/test_deep_chain_v150.py` drives a store whose `cancel` fails for one task and asserts the remaining tasks were still cancelled and the first error still surfaced. `tests/registry/test_hot_reload_on_load.py` covers all three D-123 rules through `_handle_file_change`: the reloaded revision's `on_load` actually running; a revision whose `on_load` raises never becoming visible (the ordering proof — a failing load can only leave the revision absent if the load precedes the publish) while the previous instance is restored and re-loaded; the module staying unavailable when the restoring load also fails; and `apcore.registry.module_load_failed` reaching a wired emitter. All five fail against the unfixed code.

- The v1.50.0 deep-chain decisions and the verified D11 findings are pinned in `tests/test_deep_chain_v150.py`, each one RED against the unfixed code first: the client/executor registry split through `register` → `call`, `list_modules`, `disable` and the `registry` attribute (CLI-1) and `discover()` against a configured `extensions.root` plus `_scan_params` (CLI-3); a caller-supplied `time.time()` deadline actually firing, a reused Context getting a fresh budget, and a deserialized Context with a non-empty `call_chain` recomputing (D-99 / D-101 / D-102); a recording policy subclass proving `validate()` hands it no `_approval_token`, and rule 6's "the call site MUST NOT change the verdict" as the control (APR-2); both `$ref` layouts loading from one loader, with an unresolvable pointer still raising (D-104); a step that outruns its `timeout_ms` reaching `on_step_error`, honouring `ignore_errors`, resuming on a recovery value and reaching middleware `on_error`, with the public `ModuleTimeoutError` as a regression guard (EXE-006); `replace` / `configure_step` against a sibling's name, against their own name, and the name index still anchoring `insert_after` correctly afterwards (EXE-007); `TaskStoreError`'s export, code, A17 reservation and propagation through a host store, with the in-memory store as the control (D-92); a mutated `roles` list (IDN-2); four malformed wire identities (CTX-1); the projection absent from the module-visible Context and two different projections deciding two checks on one Context (CTX-2); an excluded event not reaching a wrapped subscriber (EVT-001); a generated id with no heap address in it (EVT-004); the three `idempotent` values (SYS-19); a bulk reload that restores nothing (SYS-17); and the whole Prometheus exposition parsing against `[a-zA-Z_:][a-zA-Z0-9_:]*` (OBS-007).

- `tests/test_async_tasks_spec.py::test_taskstore_save_error_store_unavailable` was a `@pytest.mark.skip` naming the missing symbol; it now exercises `TaskStoreError` for real. `tests/test_global_timeout.py` and `tests/test_executor_stream.py` measured `global_deadline` against `time.monotonic()` — both sides were internally consistent, which is why the public `Context.create(global_deadline=...)` parameter was unusable and the suite stayed green; they now use `time.time()` per D-99. `tests/test_acl_argument_scoped_approval.py`'s three projection-location cases read the `PipelineContext` rather than the execution `Context` (CTX-2).

- The v1.49.0 deep-chain decisions are pinned case by case: the executor entry guard at, above and below `MAX_MODULE_ID_LENGTH` (D-75); `Registry.describe` for a string override, a structured override and a module with no `describe()` at all, asserting no repr leaks into the envelope (D-77); the `register` step order via a module that is malformed, validator-rejected and duplicate at once, plus the stateful-validator side effect (D-86); the deprecation-warning cadence across repeated reads, a new version, two registry instances and an unregister (D-89); and `remove_rule`'s dedupe clearing, with a failed removal as the control (D-88). `tests/test_conformance.py` maps the `call_chain` fixture's `INVALID_LIMIT` case label onto the `GENERAL_INVALID_INPUT` code D-84 now requires — the fixture itself still describes the pre-D-84 "each SDK rejects with its own idiomatic signal" state and needs a spec-repo update.

- `FileOverridesStore`'s degradation paths are covered: `OSError` on read, a file that is neither valid YAML nor valid JSON, the YAML-error-then-JSON fallback, a non-mapping YAML root, `OSError` from `mkdir` and from `mkstemp`, a failing `os.replace` (including the tempfile cleanup and a cleanup whose own `unlink` fails), and the `.path` property. The module goes from 64% to 100% line coverage, and every case asserts the degradation its own docstring documents — returns `{}` / logs / returns without raising.
...
- **A failing ACL audit callback no longer changes the access decision** (spec v1.45.0 §6.3.2 req 3, D-66). **This is a behaviour change, and it is a fix.** Measured before it: a raising `audit_logger` propagated out of `ACL.check()` and turned an **allowed** call into an error. Auditing is a side channel and does not hold a veto over access. Delivery failures are now contained and reported once per sink configuration.
  The ACL file's `audit:` block gained a delivery contract with it: declaring it activates a default sink emitting `apcore.acl.audit` with all thirteen §6.3.1 fields as structured data; a supplied callback is the one effective sink and is never narrowed by the block; `include_denied: false` warns; `reload()` refreshes the block and preserves the callback.

- **`_config.allow_unknown: false` now drops unregistered namespaces** (spec v1.46.0 §9.6.3 reqs 1–4, D-69). **Read this before upgrading**: it is the only change here that makes a `get()` which returned a value return `None`. It fires only for a configuration that explicitly writes `allow_unknown: false` — the default is `true` — and what changes for that configuration is that it finally gets the published contract instead of a no-op. `allow_unknown: true` now emits the warning the same specification row has always required. Namespace mode only: a legacy document has no namespaces.

### Fixed

- **Four pattern-valued surfaces reached for whichever matcher Python happened to offer, so one declared pattern type had a different dialect in each SDK (PROTOCOL_SPEC §9.2.3 "Algorithm A25", spec v1.37.0, [apcore#116](https://github.com/aiperceivable/apcore/issues/116), [apcore#117](https://github.com/aiperceivable/apcore/issues/117)).** `bindings.py` used `Path.glob`, which expands character classes A25 does not have and whose `*` does not cross a separator, though the pattern is matched against a *filename*; `obs.redaction.sensitive_keys` used `fnmatch` and treated `[` as a glob trigger, so `[!p]assword` was read as a negated character class when under §10.6.1 it contains neither `*` nor `?` and is therefore a plain substring; event patterns (`event_pattern`, `include_events`, `exclude_events`) and `system.control.reload_module`'s `path_filter` used `fnmatch`, where `[em]` is a class. New `apcore.utils.pattern.match_glob` implements A25 and now serves all four: exactly two metacharacters — `*` for zero or more characters, crossing `.` and `/`, and `?` for exactly one — with every other character a literal, `[`, `]`, `{`, `}`, `\`, `!`, `^` and `-` included, no escape character, anchored to the whole value, and no input that raises. It was verified exhaustively against a reference implementation over the `*`/`?` alphabet. It is deliberately **not** merged with `match_pattern` (Algorithm A08, which matches module IDs): §9.2.3 requirement 5 gives the reason and it is an authorization one — promoting `?` into A08 would widen `allow` rules that are inert in every deployed policy today. `RedactionConfig.field_patterns`, the legacy programmatic surface beside `sensitive_keys`, takes the same matcher and stays case-**sensitive** as it has always been; only the dialect changed. Drivers for the new `glob_matching.json` conformance fixture and for the discriminating cases added to four existing fixtures land with it.

- **An ACL pattern containing `?` loaded clean, matched nothing, and said nothing (PROTOCOL_SPEC §6.2.2, spec v1.37.0, [apcore#116](https://github.com/aiperceivable/apcore/issues/116)).** A08 has `*` as its only metacharacter and §2.7 forbids `?` in a module ID, so a rule written with one is silently inert — a `deny` that guards nothing, or an `allow` that never fires and sends its author looking somewhere else. The expectation is not unreasonable: `?` *is* a wildcard in every other pattern surface the specification defines (A25, §9.2.3). `ACL.load()` now warns for each such pattern, naming the rule index, the field and the pattern as written, and `validate_rules()` reports the rule when *every* pattern in a `callers` / `targets` array carries one — the condition under which the array as a whole can match nothing. Clause 1: loading **MUST NOT** fail on it, and it does not. Clause 3: the meaning is unchanged — `?` stays a literal, the rule keeps matching nothing, and no decision moves. Promoting it to a wildcard instead would widen `allow` rules that are inert in every deployed policy today, which is the one direction an authorization matcher must not move without an operator's consent.

- **An uncompilable `obs.redaction.regex_patterns` entry was discarded in silence, once per log record (PROTOCOL_SPEC §9.2.3 requirement 6d and §10.6.1, [apcore#117](https://github.com/aiperceivable/apcore/issues/117)).** A bare `except re.error: continue` in `apcore.utils.redaction` re-failed on every line it was asked to redact and reported nothing, leaving an operator-authored redaction rule that redacts nothing — indistinguishable, from the outside, from one that works, and on this surface the difference is credentials in plaintext. Each pattern is now compiled once and a failure reported once, naming the pattern and the engine's own error. The deploy-time half is new as well: `Config.validate()` reports every `obs.redaction.regex_patterns` entry that does not compile, with its index, so a rule that redacts nothing is visible before it matters rather than never. Note the asymmetry requirement 6d is written around — Python's engine accepts lookaround and backreferences, so this check reports almost nothing *here*; it is the Rust `regex` crate and JavaScript's `RegExp` that reject the portable-looking patterns, and each SDK reports what **its own** engine cannot compile. Requirement 6c, the portable subset, is the author-facing half of the same problem.

- **`stream.max_merge_depth` was declared, defaulted and read by nothing — the module constant was the contract (PROTOCOL_SPEC §5, [apcore#118](https://github.com/aiperceivable/apcore/issues/118)).** §5 requires the streaming deep-merge depth to be capped and calls 32 "the canonical default", wording that implies an override. There was none: the key is registered in all three key surfaces and carries 32 in `schemas/defaults.schema.json`, and no code path read it. Measured before the fix: with the key set to `4`, a depth-10 merge still recursed and only depth-33 reached the cap. `Executor.stream()` now resolves the key and passes it to `_deep_merge`. A non-positive or non-integer value falls back to 32 rather than disabling the cap — the cap exists to prevent stack exhaustion from adversarial chunk shapes, so a misconfiguration must not remove it, and that is the one direction this change must not open. `_deep_merge`'s new `max_depth` parameter is keyword-only and defaults to the canonical 32, so every existing caller, this suite and the conformance driver included, is unaffected.

- **The four per-group `sys_modules` registration flags were read by no code path (PROTOCOL_SPEC §9.15.3 as revised in v1.17.0, `schemas/sys-modules.schema.json`, [apcore#118](https://github.com/aiperceivable/apcore/issues/118)).** The schema declares them key by key — "Whether `system.health.summary` and `system.health.module` are registered", and so on for `manifest`, `usage` and `control` — and §9.15.3 states the split: `sys_modules.enabled` is the master switch that activates the section, and the per-group flags select *which* modules register once activation has happened. All four default to `true`, so a flag narrows and never widens. Measured before the fix: all four `false` and all four `true` both registered the same six `system.*` modules, while the master switch worked — a section that is half alive, which is why a smoke test of it passes and why this survived. `control.enabled` is the consequential one: it gates the Level 2 write plane (§6.7), the three approval-gated `system.control.*` modules, so an operator writing `control.enabled: false` to keep that surface off a deployment had expressed exactly the intent the key exists for and got all three modules anyway. Measured after: nine modules with everything on, six with `control` off and zero `system.control.*`, and exactly the two health modules with `manifest`, `usage` and `control` off. With `control.enabled` off no `ToggleState` is created during registration — `toggle_feature` owns it, and the caller's `toggle.*` overrides have nothing to apply to.

### Added

- **A configured `pipeline:` section is applied** (spec v1.43.0 §5.16 reqs 6–7, [apcore#118](https://github.com/aiperceivable/apcore/issues/118) D-72). `build_strategy_from_config` always existed; its first parameter was a dict the *caller* supplied, and nothing extracted that dict from a loaded `Config`, so `pipeline: remove: [acl_check]` in `apcore.yaml` left all eleven steps in place and a declared custom step silently never ran. Failing to *remove* a step is fail-safe; failing to *insert* one is not.
  `build_strategy_from_config` was widened to forward `policy`, `event_emitter` and `toggle_state` — without that, a configured `pipeline:` would have silently dropped the `ExecutionPolicy`. Removing `acl_check` or `approval_gate` warns once per configuration load.

- **The five `observability.tracing.*` keys are wired** (spec v1.44.0 §10.1.1, D-68 C′) — `enabled` installs a `TracingMiddleware`, `exporter` selects it by name, `strategy` and `sampling_rate` reach the sampling decision, `otlp_endpoint` reaches the OTLP exporter. `observability/tracing_config.py` is the builder. `jaeger` warns and installs **nothing** rather than substituting another exporter; so does `otlp` when the `opentelemetry` extra is absent, because a middleware whose exporter discards every span is worse than none.
  `OTLPExporter` now retains `_endpoint`: it passed the value to the OTel exporter and kept nothing, so "the endpoint reached the exporter" was unobservable to a test and to an operator alike.

- **`ACL.load(path, audit_logger=…)`** (spec v1.45.0 §6.3.2, D-66). Without it the combination the contract describes — load from a file, audit through a callback — had no supported path between them.

- **`extensions.roots` is read** (spec v1.46.0, D-70), both element shapes, with namespace isolation. `scan_multi_root` could always do the work; nothing extracted the list from a `Config` to reach it. A one-element `roots` list is namespaced like an n-element one.

- **`id_map.overrides` is read at registry construction** (spec v1.46.0, D-71). The ID-map mechanism was implemented and the map arrived only through `Registry(id_map_path=…)`; a document pointing the key at a valid map still registered the underived ID. A relative value follows `extensions.root`'s resolution base, not `acl.root`'s — [apcore#113](https://github.com/aiperceivable/apcore/issues/113) leaves that base open and this does not pre-empt it.


- **The six `validation.*` limits, every one of them unconstrained by default (PROTOCOL_SPEC §9.1.2, spec v1.38.0, [apcore#118](https://github.com/aiperceivable/apcore/issues/118)).** The keys were registered in all three key surfaces and read by none of them, and what filled the gap was a set of numbers no two documents agreed on — six of them for `description` alone. The decision is that apcore does not impose limits on the content its users author; it offers them. All six are configurable and all six check nothing until an operator sets them, so the change is purely additive: every binding file, module and pipeline that loaded before it still loads after it. `validation.binding.description_max_length` and `validation.binding.documentation_max_length` are checked in characters, not bytes (requirement 4), against each `BindingEntry`, whose schema carries exactly those four fields; `validation.binding.tags_pattern` and `validation.binding.version_require_semver` are the two **format** keys and they default off for that reason specifically — shipping them with their previously declared restrictive defaults would not have been "implementing a key", it would have added to every project whose tags are capitalised or hyphenated a rejection that does not exist today. `validation.pipeline.step_name_max_length` and `validation.pipeline.timeout_ms_max` are applied where the authored values enter, when the `pipeline:` section is parsed (requirement 3); `remove:` is deliberately not name-checked, because those names identify steps that already exist rather than naming new ones, so a limit there would reject a request to remove a step the operator did not author. When a limit *is* configured it is enforced, not warned about — the operator asked for a limit, and a limit that only warns is the `regex_patterns` failure of §10.6.1 in another place; a `tags_pattern` that does not compile is likewise refused rather than skipped (§9.2.3 requirement 6d). The SemVer grammar is written out as a literal from semver.org so three implementations cannot invent three, and note that `1.0` does not match it: the patch component is required. `BindingLoader.load_bindings` takes an optional third `config` argument to supply the limits; omitting it, as every caller before this release did, checks nothing. Only `validation.binding.version_require_semver: false` enters `_DEFAULTS` — the other five are unconstrained *as* `None`, and `None` is the absence of a default, which is what `schemas/defaults.schema.json` and `config_key_governance.json` pin.

### Deprecated

- **`acl.default_effect` in `apcore.yaml`** (spec v1.47.0 §9.1.3 req 3, D-73). An ACL's default effect is read from the **ACL file**; this twin reaches nothing. Measured: `allow` here, against an ACL file that omits the key, yields **deny** — fail-closed, which is why it went unremarked. Migrate to the ACL file's `default_effect`. Removed at v2.0.

- **`Context.logger`** ([apcore#121](https://github.com/aiperceivable/apcore/issues/121)). It has no configuration door and its output is fixed at stderr / `info` / JSON, bypassing whatever the host configured. Use the host application's own logger, carrying `trace_id` explicitly. `ObsLoggingMiddleware` is **not** the migration target — it emits apcore's execution events, a different facility. Removed at v2.0.

- **`logging.level` and `logging.format`** are withdrawn with no replacement key (spec v1.48.0, D-67). apcore does not own the host's logging policy, and `ContextLogger` does not route through the host's logging framework at all — `logging.basicConfig()` does not change a byte of its output.

- **Ten declared configuration keys that reach no consumer in any apcore SDK (PROTOCOL_SPEC §9.2.4 / §9.2.4.1, spec v1.39.0, [apcore#118](https://github.com/aiperceivable/apcore/issues/118)).** `observability.tracing.enabled` / `.sampling_rate` / `.exporter`, `observability.metrics.enabled` / `.exporter`, `logging.level` / `.format` and `acl.audit.enabled` / `.include_denied` / `.log_level` are schema-declared, environment-overridable, accepted under `_config.strict`, documented with defaults — and inert. This opens their removal window and **changes no behaviour**: they still parse, still validate, still answer `get()`, and are still accepted under `_config.strict: true`. Withdrawing them could not be a plain deletion, because all ten are accepted under strict mode today — measured — so removing a declared key would turn a currently-valid configuration into a rejected one; §13.2 sets a two-minor floor and §13.4 restates it for `remove_field`, which makes the shape *keep parsing → warn once per load → remove no earlier than 2.0*. The check is driven by the **declared** document and never by the merged view (requirement 2): every one of the ten carries a default, so a merged-view check would fire for every configuration ever loaded — the blanket warning §9.2.2 rejects, which trains an operator to ignore the one notice that matters. Emission goes through `_emit_per_load` rather than `warnings.warn` for the reason that helper already documents: a once-per-process flag makes emission order-dependent, so a later affected document goes silent. The ACL-file half needed new code. `acl-config.schema.json` declares an `audit` block byte-equivalent to `acl.audit.*`, and deleting it from the schema would produce no signal at all — no implementation validates an ACL file against that schema, and `ACL.load` parses into an open mapping, so any unknown root key is dropped in silence (verified: a file with an `audit:` block and a file with a nonsense key both loaded without a word). The notice therefore lives in the loader and is scoped to `audit` alone: this is a deprecation notice, not unknown-key closure for ACL files, and every other unrecognised root key keeps being ignored exactly as before. `tests/test_config_inert_key_deprecation.py` covers both halves, and asserts the silence as well as the warning — a configuration declaring none of the ten emits nothing, one declaring a live key beside them emits nothing, and an ACL file carrying some other unknown root key emits nothing.
